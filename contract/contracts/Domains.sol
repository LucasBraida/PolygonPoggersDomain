...
contract Domains is ERC721URIStorage, Ownable {
    ...
    function register(string calldata name) public payable {
        ...
        domainToTokenID[name] = newRecordId;
        tokenIDToDomain[newRecordId] = name;

        _tokenIds.increment();
    }

    function registerWithRecord(string calldata name, string calldata record) public payable {
        //require(domains[name] == address(0));
        if(domains[name] != address(0)) revert AlreadyRegistered();
        //require(msg.value >= price, "Not enough Matic paid");
        if(msg.value < price) revert NotMininumPriceValue();
        ...
        // string memory finalTokenUri = string( abi.encodePacked("data:application/json;base64,", json));
        string memory finalTokenUri = generateURIJson(name, record);
    ...
    //Returns all domains ordered by tokenID
    ...
    //Returns all domains and respective records ordered by tokenID
    function getAllNamesWithRecords() public view returns (Domain[] memory) {
        uint256 numberOfTokens = _tokenIds.current();
        //string[numberOfTokens][2] memory allNamesWithRecords;
        Domain[] memory allNamesWithRecords = new Domain[](_tokenIds.current());
        for (uint256 i = 0; i < _tokenIds.current(); i++) {
            string memory _name = tokenIDToDomain[i];
            string memory _record = records[_name];
            Domain memory _domain;
            _domain.name = _name;
            _domain.record = _record;
            allNamesWithRecords[i] = _domain;
        }

        return allNamesWithRecords;
    }
