...
contract Domains is ERC721URIStorage, Ownable{
  ...
  function generateURIJson(string memory name, string memory _record) internal view returns(string memory){
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        ...
        '","attributes": [{"trait_type": "Record", "value": "',_record,'"}]}'
      )
    );
  ...
  function generateURIJson(string memory name) internal view returns(string memory){
    // Combine the name passed into the function  with the TLD
    string memory _name = string(abi.encodePacked(name, ".", tld));
    // Create the SVG (image) for the NFT with the name
    string memory finalSvg = string(abi.encodePacked(svgPartOne, _name, svgPartTwo));
  ...
  function setRecord(string memory name, string memory record) public {
      ...
      uint tokenId = domainToTokenURI[name];
      records[name] = record;
